from optimizer import optim_interpGaze
import glob
...
class Engine(object):
    ...
    def parse_args(self):
        ...
        shared_parser.add_argument('-gpu', default='7', help='the gpu index to run')
        shared_parser.add_argument('-bs', '--batch_size', type=int, default=1, help='batch size')
        shared_parser.add_argument('--data_dir',
                                   default='/home/xwh/Project/interpGaze/dataset/all',
                                   help='gaze dataset dir')
        shared_parser.add_argument('--attr',
                                   # default='Mouth_Slightly_Open@Smiling,Male@No_Beard@Mustache@Goatee@Sideburns,Black_Hair@Blond_Hair@Brown_Hair@Gray_Hair,Bald@Receding_Hairline@Bangs,Young',
                                    default='0P@15P@-15P@30P@-30P,0V@10V@-10V@15V@-15V,0H@10H@-10H@15H@-15H,L@R',
                                   #default='L@R',
    ...
    def define_optim(self, model):
        optim = optim_interpGaze.optimizer(model, self.args)
        return optim

    def load_dataset(self):
        with open('info/gaze-train.txt', 'r') as f:
            train_list = [os.path.join(self.args.data_dir, tmp.rstrip()) for tmp in f]
        with open('info/gaze-test.txt', 'r') as f:
